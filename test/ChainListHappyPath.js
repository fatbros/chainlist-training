...
contract('ChainList', function(accounts){
  ...
  it('should let us sell a first article', function() {
    return ChainList.deployed().then(function(instance) {
      ...
      assert.equal(data, 1, 'number of article must be one');
  ...
  it('should let us sell a second article', function() {
    return ChainList.deployed().then(function(instance) {
      ...
      assert.equal(data, 2, 'number of article must be two');
      ...
      assert.equal(data[4], articleDescription2, 'article description2 must be ' + articleDescription2);
  ...
  it('should buy an article', function() {
    return ChainList.deployed().then(function(instance) {
      ...
      return chainListInstance.buyArticle(1, {
        ...
        assert.equal(data.length, 1, 'should now be only 1 article left for sale');
        assert.equal(data[0].toNumber(), 2, 'article 2 should be the only article left for sale');

        return chainListInstance.getNumberOfArticles();
      }).then(function(data) {
        assert.equal(data.toNumber(), 2, 'should still be 2 articles in total');
      });
    });
  });
});
